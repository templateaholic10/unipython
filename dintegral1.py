...
def line_integral2d(f, start, end):
    ...
    if y0 == y1:
        ...
        integral = (1-dx0)*f[row0:row1+2, col0].sum() + dx0*f[row0:row1+2, col0+1].sum()
...
def _cell_grad_integrals(f, rows, cols, theta1, theta2, dx1, dy1, dx2, dy2):
    '''
    各セルにおける4つの線積分の和を返す関数
    @param f: np.ndarray((N, M)) ２次元標本点上のスカラー場＝矩形[0, M-1)x[0, N-1)上のスカラー場
    @param rows 各セルの行のインデックス
    @param cols 各セルの列のインデックス
    @param theta1 始点から各セルに入る点までの距離の，線分全体に占める割合
    @param theta2 始点から各セルを出る点までの距離の，線分全体に占める割合
    @param dx1 各セルに入る点のx変位
    @param dy1 各セルに入る点のy変位
    @param dx2 各セルを出る点のx変位
    @param dy2 各セルを出る点のy変位
    @return [[L\int(1-\theta)(d\phi/dx(\phi(\theta)))d\theta, L\int(1-\theta)(d\phi/dy(\phi(\theta)))d\theta], [L\int\theta(d\phi/dx(\phi(\theta)))d\theta, L\int\theta(d\phi/dy(\phi(\theta)))d\theta]]
    '''
    ...
    # y0に関する微分
    cells_y0 = ls*((f[rows+1, cols]-f[rows, cols])*((1-dx1)*((1-theta1)/3+(1-theta2)/6)+(1-dx2)*((1-theta1)/6+(1-theta2)/3)) + (f[rows+1, cols+1]-f[rows, cols+1])*(dx1*((1-theta1)/3+(1-theta2)/6)+dx2*((1-theta1)/6+(1-theta2)/3)))
...
def line_integral2d_grad(f, start, end):
    ...
    if start[1] == end[1]:
        ...
        integrals_cover = _cell_grad_integrals(f, np.full(colR-colL+1, row, dtype=int), np.arange(colL, colR+1), thetas[:-1], thetas[1:], 0, dy, 1, dy)
        integrals_extra_L = _cell_grad_integrals(f, np.asarray((row)), np.asarray((colL)), thetas[0], 0, 0, dy, dxL, dy)
        integrals_extra_R = _cell_grad_integrals(f, np.asarray((row)), np.asarray((colR)), 1, thetas[-1], dxR, dy, 1, dy)
        ...
        integrals_cover = _cell_grad_integrals(f, np.arange(rowB, rowT+1), np.full(rowT-rowB+1, col, dtype=int), thetas[:-1], thetas[1:], dx, 0, dx, 1)
        integrals_extra_B = _cell_grad_integrals(f, np.asarray((rowB)), np.asarray((col)), thetas[0], 0, dx, 0, dx, dyB)
        integrals_extra_T = _cell_grad_integrals(f, np.asarray((rowT)), np.asarray((col)), 1, thetas[-1], dx, dyT, dx, 1)
        ...
        dxL, dyL = xL-colL, yL-rowL # 左端点の垂直線からの変位
        dxR, dyR = xR-colR, yR-rowR # 右端点の垂直線からの変位
        ...
        y2s_nox_H = dydx*(cols_nox_H+1)+yic # 「水平線との交点を含まない列の右辺」との交点のy座標
        rows_nox_H = y1s_nox_H.astype(int) # 水平線との交点を含まない列のセルの行
        if len(cols_nox_H) > 0 and cols_nox_H[0] == colL:
            # 水平線との交点を含まない列が左端の時，延長線上で水平線をまたぐ場合があるので，補正
            rows_nox_H[0] = rowL
        dy1s_nox_H = y1s_nox_H-rows_nox_H # 「水平線との交点を含まない列の左辺」との交点のy変位
        dy2s_nox_H = y2s_nox_H-rows_nox_H # 「水平線との交点を含まない列の右辺」との交点のy変位

        integrals_nox_H = _cell_grad_integrals(f, rows_nox_H, cols_nox_H, theta1s_nox_H, theta2s_nox_H, 0, dy1s_nox_H, 1, dy2s_nox_H)
        ...
        y1s_x_H = dydx*cols_x_H+yic # 「水平線との交点を含む列の左辺」との交点のy座標
        y2s_x_H = dydx*(cols_x_H+1)+yic # 「水平線との交点を含む列の右辺」との交点のy座標
        dy1s_x_H = y1s_x_H-y1s_x_H.astype(int) # 「水平線との交点を含む列の左辺」との交点のy変位
        ...
        # (4) 立った線分
        P_B, P_T = (start, end) if start[1] < end[1] else (end, start)
        xB, yB = P_B.tolist() # 下端点＝始点
        xT, yT = P_T.tolist() # 上端点＝終点
        colB, rowB = P_B.astype(int).tolist() # 下端点の属するセル
        colT, rowT = P_T.astype(int).tolist() # 上端点の属するセル
        dxB, dyB = xB-colB, yB-rowB # 下端点の水平線からの変位
        dxT, dyT = xT-colT, yT-rowT # 上端点の水平線からの変位

        dxdy = (xT-xB)/(yT-yB) # 傾きdx/dy
        xic = xB-dxdy*yB # x切片

        colL, colR = sorted([colB, colT]) # 最左列と最右列

        Vs = np.arange(colL+1, colR+1) # 垂直線
        ys_x_V = (Vs-xic)/dxdy # 垂直線との交点のy座標
        rows_x_V = ys_x_V.astype(int) # 垂直線と交点を含む行のインデックス
        rows_nox_V = np.setdiff1d(np.arange(rowB, rowT+1), rows_x_V) # 垂直線との交点を含まない行のインデックス

        # (a) 垂直線との交点を含まないセル

        theta1s_nox_V = (rows_nox_V-yB)/(yT-yB) # 「垂直線との交点を含まない行の左辺」までの線分全体に占める割合
        theta2s_nox_V = (rows_nox_V+1-yB)/(yT-yB) # 「垂直線との交点を含まない行の右辺」までの線分全体に占める割合
        x1s_nox_V = dxdy*rows_nox_V+xic # 「垂直線との交点を含まない行の下辺」との交点のx座標
        x2s_nox_V = dxdy*(rows_nox_V+1)+xic # 「垂直線との交点を含まない行の上辺」との交点のx座標
        cols_nox_V = x1s_nox_V.astype(int) # 垂直線との交点を含まない行のセルの列
        if len(rows_nox_V) > 0 and rows_nox_V[0] == rowB:
            # 垂直線との交点を含まない行が下端の時，延長線上で垂直線をまたぐ場合があるので，補正
            cols_nox_V[0] = colB
        dx1s_nox_V = x1s_nox_V-cols_nox_V # 「垂直線との交点を含まない行の下辺」との交点のx変位
        dx2s_nox_V = x2s_nox_V-cols_nox_V # 「垂直線との交点を含まない行の上辺」との交点のx変位

        integrals_nox_V = _cell_grad_integrals(f, rows_nox_V, cols_nox_V, theta1s_nox_V, theta2s_nox_V, dx1s_nox_V, 0, dx2s_nox_V, 1)

        # (b) 垂直線との交点を含む左右のセル

        theta1s_x_V = (rows_x_V-yB)/(yT-yB) # 「垂直線との交点を含む行の下辺」までの線分全体に占める割合
        theta2s_x_V = (rows_x_V+1-yB)/(yT-yB) # 「垂直線との交点を含む行の上辺」までの線分全体に占める割合
        thetaMs_x_V = (Vs-xB)/(xT-xB) # 「垂直線との交点」までの線分全体に占める割合
        dyMs_x_V = ys_x_V-rows_x_V # 垂直線との交点のy変位
        x1s_x_V = dxdy*rows_x_V+xic # 「垂直線との交点を含む行の下辺」との交点のx座標
        x2s_x_V = dxdy*(rows_x_V+1)+xic # 「垂直線との交点を含む行の上辺」との交点のx座標
        dx1s_x_V = x1s_x_V-x1s_x_V.astype(int) # 「垂直線との交点を含む行の下辺」との交点のx変位
        dx2s_x_V = x2s_x_V-x2s_x_V.astype(int) # 「垂直線との交点を含む行の上辺」との交点のx変位

        if dxdy > 0:
            # 傾き正の場合
            integrals_x_V_L = _cell_grad_integrals(f, rows_x_V, Vs-1, theta1s_x_V, thetaMs_x_V, dx1s_x_V, 0, 1, dyMs_x_V) # 左のセル

            integrals_x_V_R = _cell_grad_integrals(f, rows_x_V, Vs, thetaMs_x_V, theta2s_x_V, 0, dyMs_x_V, dx2s_x_V, 1) # 右のセル
        else:
            # 傾き負の場合
            integrals_x_V_L = _cell_grad_integrals(f, rows_x_V, Vs-1, thetaMs_x_V, theta2s_x_V, 1, dyMs_x_V, dx2s_x_V, 1) # 左のセル

            integrals_x_V_R = _cell_grad_integrals(f, rows_x_V, Vs, theta1s_x_V, thetaMs_x_V, dx1s_x_V, 0, 0, dyMs_x_V) # 右のセル

        # (c) 端点の外側を引く

        thetaBB = (rowB-yB)/(yT-yB) # 下外側端点の線分に占める割合(<0)
        xBB = dxdy*rowB+xic # 下外側端点のx座標
        dxBB = xBB-colB # 下外側端点のx変位．0-1に限らない
        integrals_extra_B = _cell_grad_integrals(f, np.asarray((rowB)), np.asarray((colB)), thetaBB, 0, dxBB, 0, dxB, dyB)

        thetaTT = (rowT+1-yB)/(yT-yB) # 上外側端点の線分に占める割合(>1)
        xTT = dxdy*(rowT+1)+xic # 上外側端点のx座標
        dxTT = xTT-colT # 上外側端点のx変位．0-1に限らない
        integrals_extra_T = _cell_grad_integrals(f, np.asarray((rowT)), np.asarray((colT)), 1, thetaTT, dxT, dyT, dxTT, 1)

        integrals = integrals_nox_V + integrals_x_V_L + integrals_x_V_R - integrals_extra_B - integrals_extra_T
...
def graph_fit(f, G, pos0, fixed=None, constraints=[]):
    ...
    最適化問題
    min \int_E(G) f dl s.t. (x_i, y_i) \in [0, M-1)x[0, N-1)
    を逐次最小二乗法(SLSQP)で解く
    @param f: np.ndarray((N, M)) ２次元標本点上のスカラー場＝矩形[0, M-1)x[0, N-1)上のスカラー場
    @param G: networkx.Graph 0-originの整数をノード名とする無向グラフ
